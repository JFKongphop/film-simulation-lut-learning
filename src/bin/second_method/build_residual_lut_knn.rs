...
const K_NEIGHBORS: usize = 5; // Number of nearest neighbors to average
...
fn main() -> Result<()> {
  println!("=== Step 8-9: Build Residual 3D LUT (K-Nearest Neighbors) ===\n");
  ...
  // Fill empty cells using K-nearest neighbors
  println!(
    "Filling empty cells using K-nearest neighbors (K={})...",
    K_NEIGHBORS
  );
  fill_empty_cells_knn(&mut lut, &count);

  // Save as .cube file
  println!("\nSaving to outputs/second_method/residual_lut_knn.cube...");
  save_cube_file(&lut, "outputs/second_method/residual_lut_knn.cube")?;

  println!("\n=== Summary ===");
  println!("Method: K-Nearest Neighbors (K={})", K_NEIGHBORS);
...
fn fill_empty_cells_knn(lut: &mut [[f32; 3]], count: &[Vec<Vec<u32>>]) {
  ...
  for idx in 0..LUT_TOTAL {
    ...
    let k = K_NEIGHBORS.min(neighbors.len());

    if k > 0 {
      // Average the K nearest neighbors
      let mut sum = [0.0f32; 3];
      for i in 0..k {
        sum[0] += neighbors[i].residual[0];
        sum[1] += neighbors[i].residual[1];
        sum[2] += neighbors[i].residual[2];
      }

      lut[idx] = [sum[0] / k as f32, sum[1] / k as f32, sum[2] / k as f32];
      filled_count += 1;
    } else {
...
fn save_cube_file(lut: &[[f32; 3]], path: &str) -> Result<()> {
  ...
  // Write header
  writeln!(file, "TITLE \"Residual LUT (KNN k={})\"", K_NEIGHBORS)?;
