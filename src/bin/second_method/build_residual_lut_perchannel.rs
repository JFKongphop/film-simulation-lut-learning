...
fn main() -> Result<()> {
  println!("=== Step 8-9: Build Residual 3D LUT (Per-Channel Variant) ===\n");
  ...
  println!("Reading outputs/second_method/matrix_tone_residual_perchannel.csv...");
  let mut reader = Reader::from_path("outputs/second_method/matrix_tone_residual_perchannel.csv")?;
  ...
  // Save as .cube file
  println!("\nSaving to outputs/second_method/residual_lut_perchannel.cube...");
  save_cube_file(&lut, "outputs/second_method/residual_lut_perchannel.cube")?;
...
fn save_cube_file(lut: &[[f32; 3]], path: &str) -> Result<()> {
  ...
  // Write header
  writeln!(file, "TITLE \"Residual LUT (Per-Channel)\"")?;
