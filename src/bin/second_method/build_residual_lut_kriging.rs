...
const VARIOGRAM_RANGE: f32 = 4.0; // Range parameter for variogram
const VARIOGRAM_SILL: f32 = 1.0; // Sill (maximum variance)
const NUGGET: f32 = 0.01; // Nugget effect (measurement error)
...
fn main() -> Result<()> {
  println!("=== Step 8-9: Build Residual 3D LUT (Kriging) ===\n");
  ...
  // Fill empty cells using Ordinary Kriging
  println!("Filling empty cells using Ordinary Kriging (spherical variogram)...");
  fill_empty_cells_kriging(&mut lut, &count);

  // Save as .cube file
  println!("\nSaving to outputs/second_method/residual_lut_kriging.cube...");
  save_cube_file(&lut, "outputs/second_method/residual_lut_kriging.cube")?;

  println!("\n=== Summary ===");
  println!("Method: Ordinary Kriging");
...
fn variogram_spherical(distance: f32, range: f32, sill: f32, nugget: f32) -> f32 {
  if distance == 0.0 {
    return 0.0;
  }
  ...
  let h_over_a = distance / range;
  nugget + sill * (1.5 * h_over_a - 0.5 * h_over_a.powi(3))
}

// Covariance function (derived from variogram)
fn covariance_from_variogram(distance: f32, range: f32, sill: f32, nugget: f32) -> f32 {
  (sill + nugget) - variogram_spherical(distance, range, sill, nugget)
}

fn fill_empty_cells_kriging(lut: &mut [[f32; 3]], count: &[Vec<Vec<u32>>]) {
  ...
  for idx in 0..LUT_TOTAL {
    ...
    // For efficiency, use only nearest neighbors (simplified kriging)
    ...
    // Simplified Ordinary Kriging: use covariance-weighted average
    // Full kriging would require solving a system of equations
    let mut weighted_sum = [0.0f32; 3];
    let mut weight_total = 0.0f32;

    for i in 0..num_neighbors {
      let (dist, neighbor_idx) = distances[i];
      let (_, neighbor_val) = &occupied[neighbor_idx];

      // Compute kriging weight using covariance
      let weight = covariance_from_variogram(dist, VARIOGRAM_RANGE, VARIOGRAM_SILL, NUGGET);
...
fn save_cube_file(lut: &[[f32; 3]], path: &str) -> Result<()> {
  ...
  // Write header
  writeln!(file, "TITLE \"Residual LUT (Kriging)\"")?;
