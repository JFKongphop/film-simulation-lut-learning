...
fn main() -> Result<()> {
  println!("📊 Comparing Residual LUT Sizes (17³ vs 33³ vs 65³) on 4 Test Images");
  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

  // Define LUT sizes to compare (size_name, suffix)
  let methods = vec![
    ("17³ (135 KB)", "nn"),      // baseline NN with 17³ LUT
    ("33³ (988 KB)", "33"),
    ("65³ (7.4 MB)", "65"),
  ];
  ...
  for img_num in &test_images {
    ...
    println!("\n┌────────────┬──────────┬───────────┬──────────┬──────────┬──────────┐");
    println!("│ LUT Size   │   MSE    │ PSNR (dB) │ Avg ΔE   │ Med ΔE   │ Max ΔE   │");
    println!("├────────────┼──────────┼───────────┼──────────┼──────────┼──────────┤");

    // Evaluate each method
    for (display_name, suffix) in &methods {
      // Load pipeline output
      let output_path = format!("outputs/second_method/final_clone_{}_{}.jpg", img_num, suffix);
      ...
      println!(
        ...
        display_name, mse, psnr, avg_de, median_de, max_de
      );

      // Store results with suffix as key
      all_results.insert((*img_num, suffix.to_string()), (mse, psnr, avg_de, median_de, max_de));
    }
    println!("└────────────┴──────────┴───────────┴──────────┴──────────┴──────────┘");
  }

  // Compute averages across all images
  let mut method_averages: Vec<(String, String, f64, f64, f32, f32, f32)> = Vec::new();
  ...
  for (display_name, suffix) in &methods {
    ...
    for img_num in &test_images {
      let (mse, psnr, avg_de, median_de, max_de) = all_results.get(&(*img_num, suffix.to_string())).unwrap();
      sum_mse += mse;
      sum_psnr += psnr;
      sum_avg_de += avg_de;
  ...
  method_averages.sort_by(|a, b| b.3.partial_cmp(&a.3).unwrap());
  ...
  for (display_name, _suffix, mse, psnr, avg_de, median_de, max_de) in &method_averages {
    println!(
      "│ {:10} │ {:8.4} │ {:9.4} │ {:8.4} │ {:8.4} │ {:8.2} │",
      display_name, mse, psnr, avg_de, median_de, max_de
    );
  ...
  println!("   Highest PSNR:  {} ({:.4} dB)", best_psnr.0, best_psnr.3);

  let best_de = method_averages
    .iter()
    .min_by(|a, b| a.4.partial_cmp(&b.4).unwrap())
    .unwrap();
  println!("   Lowest Avg ΔE: {} ({:.4})", best_de.0, best_de.4);

  println!("\n🎉 Comparison complete!");
  println!("   Evaluated {} LUT sizes on {} test images", methods.len(), test_images.len());
