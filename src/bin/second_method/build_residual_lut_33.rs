...
fn main() -> Result<()> {
  println!("=== Step 8-9: Build Residual 3D LUT (33³) ===");
  ...
  // Save as .cube file
  println!("\nSaving to outputs/second_method/residual_lut_33.cube...");
  save_cube_file(&lut, "outputs/second_method/residual_lut_33.cube")?;
...
fn save_cube_file(lut: &[[f32; 3]], path: &str) -> Result<()> {
  ...
  // Write header
  writeln!(file, "TITLE \"Residual LUT 33^3\"")?;
