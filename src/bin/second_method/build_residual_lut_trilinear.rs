...
fn main() -> Result<()> {
  println!("=== Step 8-9: Build Residual 3D LUT (Trilinear Interpolation) ===\n");
  ...
  // Fill empty cells using trilinear interpolation
  println!("Filling empty cells using trilinear interpolation...");
  fill_empty_cells_trilinear(&mut lut, &count);

  // Save as .cube file
  println!("\nSaving to outputs/second_method/residual_lut_trilinear.cube...");
  save_cube_file(&lut, "outputs/second_method/residual_lut_trilinear.cube")?;

  println!("\n=== Summary ===");
  println!("Method: Trilinear Interpolation");
...
fn fill_empty_cells_trilinear(lut: &mut [[f32; 3]], count: &[Vec<Vec<u32>>]) {
  ...
  for idx in 0..LUT_TOTAL {
    ...
    // Find bounding box of occupied cells within radius
    let mut weighted_sum = [0.0f32; 3];
    let mut weight_total = 0.0f32;

    // Search within radius for occupied cells
    for ir in r.saturating_sub(SEARCH_RADIUS)..=(r + SEARCH_RADIUS).min(LUT_SIZE - 1) {
      for ig in g.saturating_sub(SEARCH_RADIUS)..=(g + SEARCH_RADIUS).min(LUT_SIZE - 1) {
        for ib in b.saturating_sub(SEARCH_RADIUS)..=(b + SEARCH_RADIUS).min(LUT_SIZE - 1) {
          if count[ir][ig][ib] == 0 {
            continue;
          }

          // Calculate distance in each dimension
          let dr = (r as f32 - ir as f32).abs();
          let dg = (g as f32 - ig as f32).abs();
          let db = (b as f32 - ib as f32).abs();

          // Trilinear weight: product of linear interpolation weights
          let weight = (1.0 - dr / SEARCH_RADIUS as f32)
            * (1.0 - dg / SEARCH_RADIUS as f32)
            * (1.0 - db / SEARCH_RADIUS as f32);
    ...
    // If we found neighbors, use weighted average
    if weight_total > 0.0 {
      lut[idx] = [
...
fn save_cube_file(lut: &[[f32; 3]], path: &str) -> Result<()> {
  ...
  // Write header
  writeln!(file, "TITLE \"Residual LUT (Trilinear)\"")?;
